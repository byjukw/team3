# """
# D* Lite grid planning with yaw-aware initialization
# Modified to ensure the first step of the path aligns with robot yaw
# """
# import math
...
#         self.obstacles_xy = np.array([[o.x, o.y] for o in self.obstacles])
#         self.start = Node(0, 0)
#         self.goal = Node(0, 0)
#         self.U = []
#         self.km = 0.0
...
#         return any((self.obstacles_xy == [node.x, node.y]).all(1)) or \
#                any((self.detected_obstacles_xy == [node.x, node.y]).all(1))
...
#         return [add_coordinates(u, m) for m in self.motions if self.is_valid(add_coordinates(u, m))]

#     def initialize(self, start: Node, goal: Node):
#         self.start = Node(start.x - self.x_min_world, start.y - self.y_min_world)
#         self.goal = Node(goal.x - self.x_min_world, goal.y - self.y_min_world)
...
#             self.rhs[u.x][u.y] = min([self.c(u, s) + self.g[s.x][s.y] for s in self.get_neighbours(u)])
...
#     def main(self, start: Node, goal: Node, spoofed_ox: list, spoofed_oy: list, start_heading=0.0):
#         self.initialize(start, goal)
#         self.compute_shortest_path()
#         path = [self.start]

#         yaw_vec = np.array([math.cos(start_heading), math.sin(start_heading)])
#         start_world_x = self.start.x + self.x_min_world
#         start_world_y = self.start.y + self.y_min_world

#         first_moves = self.get_neighbours(self.start)
#         valid_first = []
#         for move in first_moves:
#             move_world_x = move.x + self.x_min_world
#             move_world_y = move.y + self.y_min_world
#             dx = move_world_x - start_world_x
#             dy = move_world_y - start_world_y
#             move_vec = np.array([dx, dy])
#             norm = np.linalg.norm(move_vec)
#             if norm < 1e-6:
#                 continue
#             move_vec_norm = move_vec / norm
#             dot_product = np.dot(yaw_vec, move_vec_norm)
#             if dot_product >= 0.5:
#                 valid_first.append(move)

#         if not valid_first:
#             valid_first = first_moves  # fallback if no good alignment

#         self.start = min(valid_first, key=lambda s: self.c(path[-1], s) + self.g[s.x][s.y])
#         path.append(self.start)

#         while not compare_coordinates(self.start, self.goal):
#             self.start = min(self.get_neighbours(self.start), key=lambda s: self.c(self.start, s) + self.g[s.x][s.y])
#             if self.g[self.start.x][self.start.y] == math.inf:
#                 return False, [], []
#             path.append(self.start)

#         px = [p.x + self.x_min_world for p in path]
#         py = [p.y + self.y_min_world for p in path]
#         return True, px, py
